...
export default function CandidateDashboard() {
  ...
  const getNextActionHint = (p: CandidateProfile | null): string => {
    ...
    if (!p.skills || p.skills.length < 3) return "Add more skills to improve your match rate";
    if (p.recommendationsCount === 0) return "Get recommendations to stand out from other candidates";
    if (!p.profileVisible) return "Make your profile visible to start receiving interest";
  ...
  const getPrimaryCTA = (p: CandidateProfile | null) => {
    ...
    if (!p.profileVisible) {
      return (
        <Button onClick={handleToggleVisibility} disabled={isTogglingVisibility}>
  ...
  const handleToggleVisibility = async () => {
    ...
    try {
      const res = await api.patch<CandidateProfile>('/candidates/profile', {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="flex items-center gap-4">
              <div className="relative shrink-0">
                <svg className="w-16 h-16 transform -rotate-90" viewBox="0 0 64 64">
                  ...
                  <circle
                    cx="32" cy="32" r="28"
                    stroke={completionPercent >= 70 ? '#10B981' : completionPercent >= 40 ? '#3B82F6' : '#F59E0B'}
                    strokeWidth="4"
                    fill="none"
              ...
              <div>
                ...
                <p className="font-semibold text-gray-900">{getCompletionLabel(completionPercent)}</p>
          ...
          <Card className="group hover:border-blue-200 transition-colors">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Profile Views</p>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  {(profile?.recommendationsCount || 0) > 0 ? (
                    <p className="text-2xl font-bold text-gray-900">{profile?.recommendationsCount}</p>
                  ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          ...
          <Card>
            ...
            <div className="space-y-4">
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      profile?.profileVisible ? 'translate-x-6' : 'translate-x-1'
                    }`}
                  />
                </button>
              </div>
              ...
              {profile?.desiredRole && (
                <div>
                  <span className="text-gray-500 block text-xs uppercase tracking-wide mb-1">Desired Role</span>
                  <span className="font-medium text-gray-900">{profile.desiredRole}</span>
                </div>
              )}

              {profile?.skills && profile.skills.length > 0 && (
                <div>
                  <span className="text-gray-500 block text-xs uppercase tracking-wide mb-2">Top Skills</span>
              ...
              {!profile?.cvFileName && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-yellow-800">CV not uploaded</p>
